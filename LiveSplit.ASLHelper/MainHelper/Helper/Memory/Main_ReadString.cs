...
namespace ASLHelper
{
    public partial class Main
    {
        ...
        {
            TryReadString(out var result, length, stringType, Game?.MainModuleWow64Safe(), baseOffset, offsets);
            return result;
        }

        public string ReadString(int length, ReadStringType stringType, string moduleName, int baseOffset, params int[] offsets)
        {
            TryReadString(out var result, length, stringType, GetModule(moduleName), baseOffset, offsets);
            return result;
        }
        ...
        {
            TryReadString(out var result, length, stringType, module, baseOffset, offsets);
            return result;
        }

        public string ReadString(int length, ReadStringType stringType, IntPtr baseAddress, params int[] offsets)
        {
            TryReadString(out var result, length, stringType, baseAddress, offsets);
            return result;
        }
        #endregion
